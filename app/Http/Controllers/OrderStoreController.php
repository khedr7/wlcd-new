...
class OrderStoreController extends Controller
{
    ...
    public function orderstore($txn_id, $payment_method, $sale_id = NULL, $file = NULL, $payment_status = NULL, $auth_user_id = NULL)
    {
        ...
        foreach ($carts as $cart) {
            ...
            $lastOrder = Order::orderBy('created_at', 'desc')->where('order_id', '!=', null)->where('order_id', '!=', '')->first();
            ...
            if ($created_order) {
                if ($course_id != null) {
                    ...
                    $bundle = BundleCourse::where('id', $bundle_id)->first();
                    $admins = User::where('role', 'admin')->where('status', 1)->get();
                    $instructor = User::where('id', $bundle->user_id)->where('status', 1)->first();
                    $unique_instructor = 1;
                    ...
                    foreach ($admins as $admin) {
                        ...
                        $body = 'A new enrollment request has been added to course: ' . $bundle->title;
                        $notification = NewNotification::create(['body' => $body]);
                        $notification->users()->attach(['user_id' => $admin->id]);
                        if (isset($admin->device_token)) {
                            $this->send_notification($admin->device_token, 'Course Enrollment', $body);
                    ...
                    if ($unique_instructor == 1 && isset($instructor)) {
                        $body = 'A new enrollment request has been added to bundle course: ' . $bundle->title;
                        $notification = NewNotification::create(['body' => $body]);
                        $notification->users()->attach(['user_id' => $instructor->id]);
                        if (isset($admin->device_token)) {
                            $this->send_notification($admin->device_token, 'Course Enrollment', $body);
