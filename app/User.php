...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    protected $fillable = [
        ...
        'state_id', 'country_id', 'gender', 'pin_code', 'status', 'verified', 'role', 'married_status',
        'user_img', 'detail', 'braintree_id', 'fb_url', 'twitter_url', 'youtube_url', 'linkedin_url', 'email_verified_at',
        'code', 'google_id', 'facebook_id', 'amazon_id', 'gitlab_id', 'linkedin_id', 'twitter_id', 'jwt_token',
        'zoom_email', 'referred_by', 'affiliate_id', 'google2fa_secret', 'google2fa_enable', 'remember_token',
        'vacation_start', 'vacation_end', 'age', 'device_token', 'practical_experience', 'basic_skills',
        'professional_summary', 'scientific_background', 'courses'
    ];
    ...
    {
        return $this->hasMany('App\Order', 'instructor_id');
    }
    ...
    {
        return $this->hasMany(FavCategory::class);
    }

    // one to many
    public function favSubcategoies()
    {
        return $this->hasMany(FavSubcategory::class);
    }

    // many to many
    public function newNotifications()
    {
        return $this->belongsToMany(NewNotification::class, 'notification_user', 'user_id', 'notification_id')
            ->withPivot('status');;
    }
