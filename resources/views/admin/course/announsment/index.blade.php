			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="" class="displaytable table table-striped table-bordered w-100">
						<thead>
							<tr>
								<th><input id="checkboxAll8" type="checkbox" class="filled-in" name="checked[]" value="all" />
							...
							@foreach ($cor->announsment as $an)
								<tr>
									...
									<td>
										...
										<input type="checkbox" form="bulk_delete_form8" class="filled-in material-checkbox-input8" name="checked[]"
											value="{{ $an->id }}" id="checkbox{{ $an->id }}">
											...
											<div class="modal-dialog modal-sm">
												...
												<div class="modal-content">
													...
													<div class="modal-body text-center">
														...
														<p>
															{{ __('Do you really want to delete selected item ? This process
															                                          cannot be undone') }}.
														</p>
													</div>
													<div class="modal-footer">
														<form id="bulk_delete_form8" method="post" action="{{ route('announcement.bulk_delete') }}">
															@csrf
															@method('POST')
									...
									<td>{{ str_limit($an->getTranslation('announsment', Session::get('changed_language'), false), $limit = 70, $end = '....') }}</td>
									<!-- ================================== -->
									<td>
										<button type="button"
											class="btn btn-rounded {{ ($an->status == '1' ? 'checked' : '') ? 'btn-success-rgba' : 'btn-danger-rgba' }}"
									...
									<td>
										<div class="dropdown">
											...
											<div class="dropdown-menu" aria-labelledby="CustomdropdownMenuButton1">
												@can('announcement.edit')
													<a class="dropdown-item" href="{{ url('announsment/' . $an->id) }}"><i
										...
										<div class="modal fade bd-example-modal-sm" id="delete{{ $an->id }}" tabindex="-1" role="dialog"
											aria-hidden="true">
											<div class="modal-dialog modal-sm">
												<div class="modal-content">
													<div class="modal-header">
													...
													<div class="modal-body">
														...
														<p>{{ __('Do you really want to delete') }} <b>{{ $an->courses->title }}</b> ?
	...
	<div class="modal fade" id="myModalabcdef" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
					...
					<div class="panel panel-sum">
						<div class="modal-body">
							<form id="demo-form2" method="post" action="{{ route('announsment.store') }}" data-parsley-validate
								...
								<div class="row">
                  <div class="col-md-12">
                    <label for="announsment_en">{{ __('adminstaticword.English Announcement') }}:<sup class="redstar">*</sup></label>
                    <textarea required name="announsment_en" id="announsment_en" rows="2" class="form-control select2"
                     placeholder="{{ __('adminstaticword.English Announcement') }}"></textarea>
                  </div>
                </div>
                <br>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <label for="announsment_ar">{{ __('adminstaticword.Arabic Announcement') }}:<sup class="redstar">*</sup></label>
                    <textarea required name="announsment_ar" id="announsment_ar" rows="2" class="form-control select2"
                     placeholder="{{ __('adminstaticword.Arabic Announcement') }}"></textarea>
                  </div>
                </div>
                <br>
