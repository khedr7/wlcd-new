						...
						<tbody id="sortable-chapter">
							...
							@foreach ($coursechapter as $cat)
								<tr class="sortable row1" data-id="{{ $cat->id }}">
									<?php $i++; ?>
									<td>
										<input type="checkbox" form="bulk_delete_form3" class="filled-in material-checkbox-input check"
									...
									<td>{{ $cat->getTranslation('chapter_name', Session::get('changed_language'), false) }}</td>
									<td>
										<label class="switch">
											<input class="slider" type="checkbox" data-id="{{ $cat->id }}" name="status"
												{{ $cat->status == '1' ? 'checked' : '' }} onchange="courcechapter('{{ $cat->id }}')" />
											...
											<div class="modal-dialog modal-sm">
												<div class="modal-content">
													...
													<div class="modal-body">
														...
														<p>{{ __('Do you really want to delete') }} <b>{{ $cat->courses->title }}</b> ?
				...
				<div class="panel panel-sum">
					<div class="modal-body">
						<form id="demo-form2" method="post" action="{{ route('coursechapter.store') }}" data-parsley-validate
							...
							<div class="row">
								<div class="col-md-12">
									<label for="exampleInputTit1e">{{ __('adminstaticword.English ChapterName') }}: <span class="redstar">*</span>
									</label>
									<input type="text" placeholder="{{ __('adminstaticword.English ChapterName') }}" class="form-control" name="chapter_name_en"
										id="exampleInputTitle" value="" required>
								</div>
							</div>
							<br>
							<div class="row">
								<div class="col-md-12">
									<label for="exampleInputTit1e">{{ __('adminstaticword.Arabic ChapterName') }}: <span class="redstar">*</span>
									</label>
									<input type="text" placeholder="{{ __('adminstaticword.Arabic ChapterName') }}" class="form-control" name="chapter_name_ar"
										id="exampleInputTitle" value="" required>
								</div>
							</div>
							<br>
								...
								<div class="row">
									...
									<div class="col-md-12" style="display: none;" id="dripdays1">
										<div class="form-group">
											...
											<input type="number" min="1" class="form-control" value="{{ old('drip_days') }}"
												name="drip_days">
