		...
		<div class="card m-b-30">
			<div class="card-header">
				@can('quiz-topic.delete')
					<button type="button" class="btn btn-danger-rgba mr-2 " data-toggle="modal" data-target="#bulk_delete10"><i class="feather icon-trash mr-2"></i>{{ __('Delete Selected') }} </button>
				@endcan
				@can('quiz-topic.create')
					<a data-toggle="modal" data-target="#myModaltopic" href="#" class="btn btn-primary-rgba"><i class="feather icon-plus "></i>{{ __('Add Quiz') }} </a>
				@endcan
			</div>
			<div class="card-body">
				<div class="table-responsive">
					<table id="" class="displaytable table table-striped table-bordered w-100">
						<thead>
							<tr>
								<th><input id="checkboxAll10" type="checkbox" class="filled-in" name="checked[]" value="all" />
						...
						<tbody>
							...
							@foreach ($topics as $topic)
								<tr>
											...
											<div class="modal-dialog modal-sm">
												...
												<div class="modal-content">
													...
													<div class="modal-footer">
														<form id="bulk_delete_form10" method="post" action="{{ route('quiztopic.bulk_delete') }}">
															@csrf
															@method('POST')
									...
									<td>{{ $topic->getTranslation('title', Session::get('changed_language'), false) }}</td>
									...
									<td>
										<div class="dropdown">
											...
											<div class="dropdown-menu" aria-labelledby="CustomdropdownMenuButton1">
												@can('quiz-topic.edit')
													<a class="dropdown-item" href="{{ url('admin/quiztopic/' . $topic->id) }}"><i class="feather icon-edit mr-2"></i>{{ __('Edit') }}</a>
												@endcan
												@can('quiz-topic.view')
													<a class="dropdown-item" href="{{ route('questions.show', $topic->id) }}"><i class="feather icon-file-plus mr-2"></i>{{ __('Add Question') }}</a>
												@endcan
												<a class="dropdown-item" href="{{ route('answersheet', $topic->id) }}"><i class="feather icon-delete mr-2"></i>{{ __('Delete Answer') }}</a>
												<a class="dropdown-item" href="{{ route('show.quizreport', $topic->id) }}"><i class="feather icon-file mr-2"></i>{{ __('Show Report') }}</a>
												<a class="dropdown-item btn btn-link" data-toggle="modal" data-target="#deleteq{{ $topic->id }}">
													<i class="feather icon-delete mr-2"></i>{{ __('Delete') }}</a>
												</a>
											</div>
										</div>
										<div class="modal fade bd-example-modal-sm" id="deleteq{{ $topic->id }}" tabindex="-1" role="dialog" aria-hidden="true">
											<div class="modal-dialog modal-sm">
												<div class="modal-content">
													<div class="modal-header">
													...
													<div class="modal-body">
														...
														<p>{{ __('Do you really want to delete') }} <b>{{ $topic->title }}</b>?
		...
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="my-modal-title">
					<b>{{ __('adminstaticword.AddQuiz') }}</b>
				</h5>
						...
						<form id="demo-form2" method="post" action="{{ url('admin/quiztopic/') }}" data-parsley-validate class="form-horizontal form-label-left">
							...
							<div class="row">
								<div class="col-md-12">
									...
									<input required type="text" placeholder="{{ __('adminstaticword.English Title') }}" class="form-control " name="title_en" id="title_en" value="">
								</div>
							</div>
							<br>
							...
							<div class="row">
								<div class="col-md-12">
									<label for="title_ar">{{ __('adminstaticword.Arabic Title') }}:<span class="redstar">*</span>
									</label>
									<input required type="text" placeholder="{{ __('adminstaticword.Arabic Title') }}" class="form-control " name="title_ar" id="title_ar" value="">
								</div>
							</div>
							<br>
							...
							<div class="row">
								<div class="col-md-12">
									<label for="description_en">{{ __('adminstaticword.English Description') }}:<sup class="redstar">*</sup></label>
									<textarea name="description_en" rows="3" class="form-control" placeholder="{{ __('adminstaticword.English Description') }}"></textarea>
								</div>
							</div>
							<br>
							<div class="row">
								<div class="col-md-12">
									<label for="description_ar">{{ __('adminstaticword.Arabic Description') }}:<sup class="redstar">*</sup></label>
									<textarea name="description_ar" rows="3" class="form-control" placeholder="{{ __('adminstaticword.Arabic Description') }}"></textarea>
								</div>
							</div>
							<br>
							...
							<div class="row">
								<div class="col-md-12">
									<label for="exampleInputTit1e">{{ __('adminstaticword.Marks') }}:<span class="redstar">*</span>
									</label>
							...
							<div class="row">
								<div class="col-md-12">
									<label for="exampleInputTit1e">{{ __('adminstaticword.QuizTimer') }}:<span class="redstar">*</span>
									</label>
